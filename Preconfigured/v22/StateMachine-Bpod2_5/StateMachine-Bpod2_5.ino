/*
  ...
  Copyright (C) 2024 Sanworks LLC, Rochester, New York, USA
...
*/
// ################################################################################
// # Bpod State Machine Firmware Ver. 22 Modded to use FSM 2.5 with Legacy PyBpod #
// ################################################################################
//
// Important!
// This firmware mod can only be compiled for FSM2.5 with the default config. Leave all macros fixed!
// This firmware will reply to the '+' op to indicate that it is FSM v2.5. 
// If v2.5, the firmware responds with 65 (character A). v2.0-2.4 will ignore the + op.
...
volatile boolean statusLEDEnabled = true; // Set to false to disable status LED
  ...
  if ((PC.available() > 0) && (smaPending == false)) { // If a message has arrived on the USB serial port
    ...
    switch (CommandByte) {
      case '6':  // Initialization handshake
        ...
        #endif
      break;
      case '+': // Op for legacy PyBpod to probe for FSM v2.5. No reply after 1s indicates v2.0-2.4
        PC.writeByte(65); // Return A
      break;
